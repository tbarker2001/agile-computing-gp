...
def scanAllArxivCategoriesRecent():
    ...
    for link in soup.findAll('a', attrs={'href': re.compile("list/")}):
        scanCategory(url + link.get('href'))
...
def scanAllArxivComputerScienceCategories2020():
    ...
    for link in soup.findAll('a', attrs={'href': re.compile("list/cs\.")}):
        ...
        print("\n " +l+" Parsed " + str(parsed))
        scanCategory(l, parsed)

def scanCategory(url,parsed):
    print("hi")
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.3'}
    req = Request(url=url, headers=headers)
    page = urlopen(req)
    ...
    for link in soup.findAll('a', attrs={'href': re.compile("/abs/")}):
            if parsed%200 == 0:
                time.sleep(60)
            print(str(c2)+', ', end = '')
            parsePaper("https://export.arxiv.org/"+link.get('href'))
            c2+=1
            parsed+=1
...
def tokenizeTitle(title):
    ...
    #remove tokens that are tagged with CC (coordinating conjunction), DT (Determiner), EX (existential there), IN (preposition of subordinating conjunction)
    #PDT, PRP, PRP$, RP, TO, UH, WP, WP$, WRB, WDT
    tagged = list(filter(lambda x:x[1]!='CC' and x[1]!='DT' and x[1]!='EX' and x[1]!='IN' and x[1]!='PDT' and x[1]!='PRP' and x[1]!='PRP$' and x[1]!='RP' and x[1]!='TO' and x[1]!='UH' and x[1]!='WP' and x[1]!='WP$' and x[1]!='WRB' and x[1]!='WDT', tagged))
